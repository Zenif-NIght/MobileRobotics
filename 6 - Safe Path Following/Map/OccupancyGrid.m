classdef OccupancyGrid < handle
    ...
    properties(Constant)
        FREE = 0 % Indicates that a location is free
        OCCUPIED = 1 % Indicates that a location is occupied
    end
    ...
    methods
        ...
        function occupied = isOccupied(obj, q)
            ...
            occupied = obj.grid(row, col) > obj.FREE;
        end
        ...
        function q = arrayIndexToPosition(obj, ind)
            % Get the array index
            [row, col] = ind2sub([obj.n_rows, obj.n_cols], ind);
            
            % Get the position
            q = obj.indexToPosition(row, col);
        end
        ...
        function [ind, valid] = positionToArrayIndex(obj, q)
            %positionToArrayIndex returns the single array-based index for
            %the position q
            
            % Get the row and column indices
            [row, col, valid] = obj.positionToIndex(q);
            
            % Convert the row and column indices to array indices
            ind = sub2ind([obj.n_rows, obj.n_cols], row, col);
        end
